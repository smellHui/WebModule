...
(function ($) {
    $.fn.date = function (options,Ycallback,Ncallback) {   
        //插件默认选项
        var that = $(this);
        var form=that.children('span');
        var docType = $(this).is('div');
        var datetime = false;
        ...
        $.fn.date.defaultOptions = {
            ...
            mode:null,                       //操作模式（滑动模式）
            event:"touchend",                    //打开日期插件默认方式为点击后后弹出日期 
            show:true
        }
        //用户选项覆盖插件默认选项   
        ...
        function refreshDate(){
            ...
            dayScroll.refresh();
             ...
             if(!resetInitDete()){
            	 yearScroll.scrollTo(0, initY*40+40*100, 100, true);
             }else
	...
	function resetIndex(){
        ...
        function resetInitDete(){//初始化日期
            if(opts.curdate){
            	return false;
            }else if(form.html()===""){
            	return false;
            }
            var temp=parseInt(form.html().substr(0,2));
            if(temp==20)
            	initY = parseInt(form.html().substr(2,2))+100;
            else
            	initY = parseInt(form.html().substr(2,2));
            initM = parseInt(form.html().substr(5,2));
            initD = parseInt(form.html().substr(8,2));
            return true;
        }
        function bindButton(){
            resetIndex();
            $("#dateconfirm").unbind('touchend').bind({
            	touchend:function () {
            ...
            $("#datecancle").bind({
            	touchend:function () {
                    $("#datePage").hide(); 
                    $("#dateshadow").hide();
                    //$("#datePlugin").css("overflow","auto");
                    //Ncallback(false);
                    //$('html,body').removeClass('ovfHiden'); //使网页恢复可滚动
                    //document.getElementById("datePlugin").addEventListener('touchmove', move, false);
        ...
        function extendOptions(){
            ...
            document.getElementById("dateshadow").addEventListener('touchmove', move, false);
        }
        function move(e){//阻止默认事件
        	e.preventDefault();
            e.stopPropagation();
        }
        //日期滑动
