...
var refresher = {
	info: {
		"pullDownLable": "下拉刷新更多...",
		"pullingDownLable": "释放加载更多...",
		"pullUpLable": "上拉加载 更多...",
	...
	init: function(parameter) {
		...
		var list = wrapper.querySelector("#" + parameter.id + " ul");//取得id 下的ul
		scroller.insertBefore(list, scroller.childNodes[0]);
		var pullDownEl=parameter.pullDownAction;
		if(pullDownEl!=undefined){
			var pullDown = document.createElement("div");//创建下拉div
			...
			var pullDownLabel = document.createElement("div");//创建 下拉 文字 div
		...
		var pullUp = document.createElement("div");//创建 上拉div
	...
	onScrolling: function(e, pullDownEl, pullUpEl, pullUpOffset) {//滚动
		if (e.y > -(pullUpOffset)) {
			if(pullDownEl!=null){
				pullDownEl.id = '';
				pullDownEl.querySelector('.pullDownLabel').innerHTML = refresher.info.pullDownLable;
	...
	onRelease: function(pullDownEl, pullUpEl) {//放开时
		if(pullDownEl!=null)
	...
	onPulling: function(pullDownEl, pullDownAction, pullUpEl, pullUpAction) {//拉动时
		if(pullDownEl!=null)
